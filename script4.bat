...
REM ******************** LOGOFF DE TODOS OS USUÁRIOS ********************

echo Desconectando todos os usuários...

for /f "skip=1 tokens=3" %%i in ('query session') do (
    if not "%%i"=="Services" (
        logoff %%i
    )
)
echo Todos os usuários foram desconectados.
